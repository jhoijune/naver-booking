...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const canceledCount = document.querySelector(
    '.reservationCount .canceled h1',
  );
  toUsedSection.addEventListener('click', event => {
    if (event.target.localName === 'button') {
      if (
        confirm('한번 취소하시면 다시 되돌릴 수 없습니다 그래도 하시겠습니까?')
      ) {
        ...
        xhr.onload = () => {
          if (xhr.status === 400) {
            ...
            if (canceledReservation.length === 0) {
              ...
              const rawAddReservationId = specificReservation.querySelector(
                '.topSection h2',
              );
              const addReservationId = parseInt(
                rawAddReservationId.innerText.slice(3),
              );
              let beforeReservationId;
              for (
                let index = 0, len = canceledReservation.length;
                index < len;
                index++
              ) {
                const rawReservationId = canceledReservation[
                  index
                ].querySelector('.topSection h2');
                const reservationId = parseInt(
                  rawReservationId.innerText.slice(3),
                );
                if (addReservationId < reservationId && index === 0) {
                  // 처음 삽입
                  canceledList.insertBefore(
                    specificReservation,
                    canceledReservation[index],
                  );
                  break;
                }
                if (
                  beforeReservationId < addReservationId &&
                ...
                if (
                  addReservationId > reservationId &&
                  index === canceledReservation.length - 1
                ) {
                  // 끝 삽입
                  canceledList.appendChild(specificReservation);
                  break;
        ...
        xhr.open(
          'PUT',
          `/api/reservations/${button.dataset.reservationInfoId}`,
        );
        xhr.send();
      }
    }
