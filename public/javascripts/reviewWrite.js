...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const commentCount = document.querySelector(
    '.writeReview .postImage .commentCount span',
  );
  ...
  imageInput.addEventListener('change', event => {
    ...
    imageMeasurement.onload = function() {
      ...
      thumbnailImage.style.backgroundImage = `url(${window.URL.createObjectURL(
        image,
      )})`;
    };
  });
  ...
  submitButton.addEventListener('click', event => {
    ...
    if (
      Number(scoreCount.value) <= 0 ||
      Number(scoreCount.value) > 5 ||
      !Number.isInteger(Number(scoreCount.value))
    ) {
